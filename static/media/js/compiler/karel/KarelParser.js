...
KarelParser.prototype.charsToLines = function() {
    var lines = this.scanner.buffer.split('\n');
    var cnt = 0;
    for (var n = 0; n < lines.length; n++) {
        cnt += lines[n].length;
        if (cnt >= this.scanner.cp) {
            return n;
        }
    }
    return 0;
...
KarelParser.statementForms["repeat"] = function(parser) {
   ...
   if (TokenScanner.getTokenType(token) != TokenScanner.NUMBER) {
      throw new Error("The repeat statement requires an integer on line " + parser.charsToLines());
   }
   parser.verifyToken(")");
...
KarelParser.prototype.readFunction = function() {
   ...
   if (!this.scanner.isValidIdentifier(name)) {
      throw new Error("\"" + name + "\" is not a legal function name on line " + parser.charsToLines());
   }
   this.verifyToken("(");
   this.verifyToken(")");
   this.verifyToken("{");
