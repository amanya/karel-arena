...
TokenScanner.prototype.verifyToken = function(expected) {
   var token = this.nextToken();
   if (token != expected) {
      var msg = "Found \"" + token + "\" when expecting : \"" + expected + "\" , on line " + this.charsToLines();
      throw new Error(msg);
   }
};
...
TokenScanner.prototype.getPosition = function() {
   ...
   throw new Error("Internal error: getPosition after two saves on line " + this.charsToLines());
};
...
TokenScanner.prototype.scanString = function() {
   ...
   while (true) {
      var ch = this.getChar();
      if (ch == "") throw new Error("Unterminated string on line " + this.charsToLines());
      if (ch == delim) break;
      if (ch == '\\') {
...
TokenScanner.prototype.charsToLines = function() {
